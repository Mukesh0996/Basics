var content = [
  ...
  const array_element = document.querySelector(".content");
  const page = document.querySelector(".pagination");
  let currentPage = 1;;
  let rows= 10;


  function display(array_element,currentPage, rows)
  {
    ...
    array_element.innerHTML = JSON.stringify(paginatedItems)
  }
  display(array_element,currentPage, rows)


  let pages = Math.ceil(content.length/rows);
  for(let i=1;i <= pages;i++)
  {
    console.log("page is",i)
    let button = pagination(i)
